...
```
1. write mission.md     describe the job, what good looks like, when to abstain
2. bootstrap            Sonnet writes the simulator from your description (~$0.05)
...
StockTiming optimizes moving average crossover parameters across simulated market regimes. It is a toy — use it to verify the loop runs end-to-end in under 5 minutes, then build your own domain.
...
# Sonnet reads your description and writes:
#   simulation.py, tournament.py, prompts/, pack.json, evals/
```
...
## Domain Packs — Install a Vertical

Domains are versioned, shareable packages. Install a community-built specialist in one command:

```bash
python run.py install freight-quoting-1.2.0.zip
python run.py install dental-front-desk-2.0.0.zip
python run.py install ecommerce-returns-1.0.0.zip
```
...
```bash
python run.py eval --domain FreightQuoting
# ✓ bid_spot_lane      score 94.2  min 80
# ✓ contract_renewal   score 88.1  min 75
# ✗ high_volatility    score 31.0  min 40   ← run more batches
# 2/3 passed (67%) — run more batches to improve
```
...
# Format: {"state": {...}, "decision": {...}, "outcome": 142.5}
python run.py import --domain FreightQuoting --file decisions.jsonl
...
MyDomain/
    ...
    ├── simulation.py           scoring function — the only file with domain knowledge  ← calibrate this
